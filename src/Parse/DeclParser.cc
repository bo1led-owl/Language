...
std::unique_ptr<AST::FnDecl> Parser::ParseFnDecl() {
  ...
  while (CurToken->IsNot(Lex::TokenKind::RParen)) {
    ...
    THROW_IF_TOKEN_IS_NOT(Lex::TokenKind::Identifier, "missing argument name declaration")
    ...
    THROW_IF_TOKEN_IS_NOT(Lex::TokenKind::Identifier, "missing argument type declaration")
